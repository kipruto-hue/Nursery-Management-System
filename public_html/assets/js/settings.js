...
document.getElementById('name-form').addEventListener('submit', busy(nameSave, async (ev) => {
  ev.preventDefault();
  await apiPost('/api/settings.php', { nursery_name: document.getElementById('nursery-name').value.trim() });
  toast('Name saved');
...
document.querySelectorAll('.w-reset').forEach(a => {
  a.addEventListener('click', async (ev) => {
    ...
    if (!yes) return;
    try {
      const data = await apiPost('/api/users.php', { action: 'reset_password', id: Number(a.dataset.id) });
      showTemp(data.temp_password);
...
document.querySelectorAll('.w-toggle').forEach(a => {
  a.addEventListener('click', async (ev) => {
    ...
    if (!yes) return;
    try {
      await apiPost('/api/users.php', { action: 'set_active', id: Number(a.dataset.id), active: activating });
      location.reload();
    } catch (e) { toast(e.message, true); }
  });
});
