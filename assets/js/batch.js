...
advanceBtn?.addEventListener('click', busy(advanceBtn, async () => {
  await apiPost('/api/batches', { action: 'set_stage', id: BATCH.id, stage: advanceBtn.dataset.stage });
  location.reload();
}));
...
writeoffBtn?.addEventListener('click', busy(writeoffBtn, async () => {
  const yes = await confirmSheet('Write off this batch?',
    ...
    'Yes, write off');
  if (!yes) return;
  await apiPost('/api/batches', { action: 'set_stage', id: BATCH.id, stage: 'written_off' });
  location.reload();
}));
...
document.getElementById('loss-form').addEventListener('submit', busy(lossSave, async (ev) => {
  ...
  const yes = await confirmSheet('Confirm loss',
    ...
    'Yes, save');
  if (!yes) return;
  await apiPost('/api/batch-events', {
    action: 'add',
    batch_id: BATCH.id,
    event_type: 'loss',
    qty,
...
document.querySelectorAll('.ev-delete').forEach(a => {
  a.addEventListener('click', async (ev) => {
    ...
    if (!yes) return;
    try {
      await apiPost('/api/batch-events', { action: 'delete', id: Number(a.dataset.id) });
      location.reload();
